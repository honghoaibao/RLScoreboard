...
dependencies {
    ...
    // Jobs Reborn - no official stability guarantee on the exact API surface the way
    // VaultAPI/LuckPerms API have, but this is the coordinate JitPack itself currently
    // recommends. See README "Jobs Reborn / AuraSkills native datasources" for the risk
    // note - if this ever breaks the build, this is the one line (plus JobsTotalLevelDataSource
    // and JobsIntegration) to remove.
    compileOnly("com.github.Zrips:Jobs:5.2.6.3")

    // AuraSkills - official Maven Central artifact, wiki-documented API, much higher
    // confidence than Jobs above.
