...
**Update, first real CI build against this work**: GitHub Actions was run against the
Public Release Upgrade work through Phase 5. Dependency resolution and every file except one
compiled cleanly on the first try - a real track record for code that had never been
compiled before. One genuine error was found and fixed: a Kotlin/Java `Enum` interop pitfall
in `AuraSkillsIntegration.kt` (`skill.name()` instead of `skill.name` - see
[README "What broke on the third real build"](README.md#what-broke-on-the-third-real-build)
for the full mechanism). Re-run CI to confirm that was the only remaining issue.
