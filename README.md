...
> **Status: Phase 1 + 2 + 3 + 4 (localization) + Integrations/Localization addendum
> (0.4.0-dev), build-tested on GitHub Actions through 0.3.0.** Every item from the original
...
> for exactly what happened. The integrations+localization addendum (version-aware
> compatibility checking, Geyser/Floodgate/WorldGuard/WorldEdit detection, the capability
> system, Japanese, dynamic locale discovery, `/rlscoreboard integrations`/`language`/
> `validate-language`) was written in an environment with **no network access to run
> `./gradlew build`** — see [Building](#building) for what that means in practice and what
> still needs a real CI run before it's trusted the way the rest of this README is.
...
range → enable or disable safely, never crash): PlaceholderAPI, Vault, LuckPerms, and
AuraSkills are registered with real, compiled-against APIs and a live capability system
(`hasCapability("economy")`, etc. — see [docs/INTEGRATIONS.md](docs/INTEGRATIONS.md#capability-system)
for the "Vault installed but no economy plugin" example this enables); Jobs Reborn, Geyser,
Floodgate, WorldGuard, and WorldEdit are detection-only (real version-aware compatibility
checking, no capability wiring yet) — see [docs/INTEGRATIONS.md](docs/INTEGRATIONS.md) for
the full compatibility matrix, per-plugin notes, and exactly what "detection-only" does and
doesn't mean.

**Commands** — `/rlscoreboard reload|version|debug|integrations`, `/rlscoreboard board
...
see the version bump note below for the integrations+localization addendum's status).
...
> **Integrations + localization addendum — not yet CI-verified.** Everything under
> `integration/geyser`, `integration/floodgate`, `integration/worldguard`,
> `integration/worldedit`, the rewritten `Integration`/`IntegrationManager`/
> `AbstractIntegration`, `LocaleValidator`, and the new `/rlscoreboard integrations|language|
> validate-language` commands were written and reasoned through carefully, but **not compiled
> in this environment** - it has no network access, so `./gradlew build` couldn't be run to
> confirm it even compiles, let alone behaves correctly against real installs of the four new
> soft-dependency plugins. They're detection-only precisely so nothing here adds a new Maven
> dependency (no new repository, no new `compileOnly` artifact - see
> [docs/INTEGRATIONS.md#how-version-detection-works](docs/INTEGRATIONS.md#how-version-detection-works)),
> which keeps the compile-time risk low, but "low risk" is not "verified". **Before trusting
> this the way the rest of this README is written, please run `./gradlew build` (or push to
> the existing GitHub Actions workflow) and treat any compiler error the same way the two
> real issues in [What broke on the first real build](#what-broke-on-the-first-real-build)
> were treated** - fix, document what broke and why, don't silently patch around it. The
> version ranges in docs/INTEGRATIONS.md for Geyser/Floodgate/WorldGuard/WorldEdit are
> similarly provisional until checked against a live server.
