...
> **Status: Phase 1 + 2 + 3 — every item from the original design spec has an
> implementation**, including the ones initially deferred as higher-risk (native Jobs/
> AuraSkills datasources) or lower-priority (NPC/TAB/GUI renderers, historical snapshots, a
> real connection pool). See [Residual risk](#residual-risk--worth-checking-first) for the
> two spots most worth a look once you can compile+run locally — nothing here was skipped,
> but a couple of pieces were written without a local build to verify against.
...
| `jobs_totallevel`         | sum of a player's level across every job    | Jobs Reborn (see risk note) |
...
**Integrations** — PlaceholderAPI, Vault, LuckPerms, Jobs Reborn, AuraSkills, all wired for
real (see the [risk note](#residual-risk--worth-checking-first) on the last two).
...
leaderboard/    LeaderboardEngine, LeaderboardManager, RankingEngine, DataSourceManager
  /datasource   StatisticDataSource, EconomyDataSource, AsyncRefreshingDataSource,
                PersistentStatDataSource, JobsTotalLevelDataSource, AuraSkillsPowerLevelDataSource
...
## Residual risk — worth checking first

Everything in this plugin was written without a local build to compile-check against (no
network access in this environment - see the note in the build section). Most of it uses
long-standing, extremely stable Bukkit/Paper API surface I have high confidence in. Two
spots don't quite meet that bar, and are worth looking at first if `./gradlew build` fails:

1. **`JobsTotalLevelDataSource`** (and `JobsIntegration`) — calls
   `Jobs.getPlayerManager().getJobsPlayer(player).getJobProgression()` and
   `JobProgression.getLevel()`/`.getJob()`. The overall call pattern is corroborated by
   multiple independent, 8+ year old public examples (strong signal it's stable), but I
   don't have a javadoc-verified confirmation of `getLevel()`/`getJob()` specifically. If
   the build fails here, this is a self-contained pair of files — delete both (or just the
   `getEntries()` body) and remove `compileOnly("com.github.Zrips:Jobs:5.2.6.3")` from
   `build.gradle.kts`; nothing else depends on it compiling.
2. **The `com.github.Zrips:Jobs:5.2.6.3` JitPack coordinate itself** — JitPack has had
   real, documented resolution flakiness on specific Jobs Reborn tags before. If the build
   fails on *resolving* this dependency (not compiling against it), it's a repository/
   version problem, not a code problem — check JitPack's build status for that tag, or drop
   the dependency and datasource as in point 1.

Everything else — AuraSkills (official Maven Central artifact + wiki-documented API,
much higher confidence), the SQLite/MySQL storage layer, the five leaderboard renderers, the
connection pool — uses either extremely well-established Bukkit/Paper API or (for AuraSkills)
directly-cited official documentation, and I'd be surprised if these need changes.
